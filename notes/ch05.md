...
- the benefit of `std::string` is that it makes its own copy of the initializer so it doesn't have to worry about what happens to said initializer after the initialization is finished, it can be destroyed/modified and the `std::string` variable remains unaffected, this comes the cost of making an expensive copy
