...
### 5.9 - more about std::string_view
- - - 
- when you are viewing a `std::string` type, and it changes its value later on in the program, the `std::string_view` variable viewing it might update the object it is looking at, or break entirely and there isn't a way to know in advance which one is going to happen
- the benefit of `std::string` is that it makes its own copy of the initializer so it doesn't have to worry about what happens to said initializer after the initialization is finished, it can be destroyed/modified and the `std::string` variable remains unaffected, this comes at the cost of making an expensive copy
- when we say that an object is an owner, we generally mean that it is the sole owner of the data it holds, sole ownership ensures it is clear who has responsibility for that data
- having multiple viewers viewing an object simultaneously is completely fine
- rephrasing the first point: *a view is dependent on the object being viewed, if the object being viewed is modified or destroyed while the view is still being used, it causes unexpected/undefined behaviour*
- a `std::string_view` that is viewing a string which has been destroyed is called a **dangling** view
- the best use case for `std::string_view` remains being used as a read-only function parameter which allows us to pass in arguments without making a copy, making it less expensive 
- note: do not initialize a `std::string_view` with a `std::string` literal, as this will leave it dangling (read the content above warning in ch 5.9 to understand why)
- you can revalidate a `std::string_view` object if it was dangling previously because the `std::string` variable was modified, you can do this by simply reassigning the `std::string_view` variable with the `std::string` variable
- you can modify what you view in a `std::string_view` by using view modification functions:
  1. `remove_prefix()` is a member function that removes characters from the left side of the view
  2. `remove_suffix()` is a member function that removes characters from the right side of the view
- to reset the view, simply reassign the `std::string_view` object with its source string
- another important use of `std::string_view` is that it can view substrings without making a copy, a **substring** is a contiguous sequence of characters within an existing string (example: in the string "Peach", the substrings can be - "each", "ach", "eac", etc.)
- this ability to view a substring comes with a consequence: a `std::string_view` may or may not be null terminated, but in almost all cases - this doesn't matter
- read the end of ch 5.9 to understand where to use `std::string` and where to use `std::string_view` correctly