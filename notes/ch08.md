...
- - -
- if you need a random number generator in more than one function or file, passing one around everywhere gets annoying real fast. and giving every function its own local static generator is overkill too, plus each one only gets called a handful of times so the quality can actually suffer
- so the answer here is just one global prng, shared across the whole program. yeah this goes against what 7.8 said about avoiding non-const globals, but this is basically the one legit exception to that rule
- the annoying part is getting a global `std::mt19937` to seed itself automatically without you having to remember to call some init function. the issue is an initializer has to be a single expression, but seeding properly needs a `random_device` and a `seed_seq`, which are statements, not an expression. the workaround is a small helper function (`generate()`) that does all that setup internally and just returns a fully seeded `std::mt19937`, and that function call becomes the initializer itself
- everything in `random.h` (the function and the global `mt`) is marked `inline`, same reason as always, lets you `#include` this into as many files as you want without odr violations, this is literally what 7.9/7.10 was building up to
- once the header's set up, using it is dead simple, `#include "random.h"` then just call `Random::get(min, max)` wherever you need a number, no setup required on your end
- there are two extra template versions of `get()` for edge cases (different types for min/max, or wanting a specific return type). notes themselves basically say don't worry about how these work yet, just know they exist for less common situations
- if you've already got your own distribution set up somewhere, you can skip `Random::get()` and just plug `Random::mt` in directly as the generator instead
- i've put up both my random.h and src file in ch08 directory, of course taken from learncpp as i'm not at the level where i can create one myself (yet)